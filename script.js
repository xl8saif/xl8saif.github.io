...
(function () {
  ...
  function validateField(field) {
    ...
    var valid = field.type === 'email'
      ? /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(field.value.trim())
      : field.value.trim().length > 0;

    field.classList.toggle('invalid', !valid);
    field.style.borderColor = valid ? '' : '#e74c3c';
  ...
  function initFileValidation() {
    var input = document.getElementById('hire-attachment');
    ...
    input.addEventListener('change', function () {
      ...
      if (file.size > MAX_FILE_SIZE || ALLOWED_EXTENSIONS.indexOf(ext) === -1) {
        input.value = '';
        if (hint) {
          hint.textContent = file.size > MAX_FILE_SIZE
            ? 'File too large. Maximum size is 10 MB.'
            : 'File type not allowed. Accepted: PDF, DOC, DOCX, TXT, XLIFF, TMX, TTX, XLSX, CSV, ZIP, RAR.';
